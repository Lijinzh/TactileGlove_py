...
# 2. Madgwick 算法增益:
#    - 控制“跟手性”。值越高，响应越灵敏，但过高可能引入抖动。
#    - 推荐范围: 0.5 ~ 5.0。从 2.0 开始尝试。
MADGWICK_GAIN = 5.0

# 3. 磁力计偏置校准值 (通过调试模式获取):
#    - 将调试模式步骤2中计算出的X, Y, Z偏置值填入此处。
MAG_BIAS = np.array([-11.70, 120.30, -43.50]) # 例: np.array([-25.5, 110.1, -50.3])
# =============================================================================
...
class PoseVisualizer3D(QMainWindow):
    ...
    def __init__(self, data_reader: SerialDataReader):
        ...
        self.setWindowTitle('6-DOF 姿态实时解算与可视化 (Madgwick)')
        ...
        self.timer = QTimer()
        self.timer.setInterval(20) # 50Hz 屏幕刷新率
    ...
    def update_pose(self):
        ...
        if raw_data is not None:
            ...
            # --- [核心] 坐标系重映射 ---
            # 这是根据你之前的调试结果确定的最佳映射方案
            acc_remapped = np.array([ acc[0], acc[1], acc[2]])
            gyro_remapped = np.array([ gyro[0], gyro[1], gyro[2]])
            # [核心] 应用磁力计偏置校准
...
def run_debug_mode():
    ...
    for _ in range(50):
        ...
        if d is not None:
            acc_samples.append(d[0:3])
        time.sleep(0.02)

    if not acc_samples:
        print("错误：未能采集到加速度数据，请检查连接。")
        reader.close()
        return
    ...
    if abs(acc_remapped[2]) > 9.0 and abs(acc_remapped[0]) < 2.0 and abs(acc_remapped[1]) < 2.0:
        ...
        print(">>> 重力校准失败！请修改代码中 `update_pose` 函数内的坐标系重映射规则，然后重试。")
        reader.close()
        return
