...
import threading
from collections import deque  # 导入 deque 用于数据存储
...
# 传感器数据存储，使用 deque 限制数据点数量
max_data_points = 100  # 最大保存的数据点数量，可以根据需要调整
sensor_data_storage = {i: deque(maxlen=max_data_points) for i in range(5)}  # 假设有最多 5 个传感器
        ...
        try:
            ...
            for sensor_idx, sensor in enumerate(all_sensors_data.sensors):
                if sensor_idx not in sensor_data_storage:
                    sensor_data_storage[sensor_idx] = deque(maxlen=max_data_points)
