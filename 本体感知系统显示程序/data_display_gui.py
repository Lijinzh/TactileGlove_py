...
class DataDisplay(QMainWindow):
    ...
    def initUI(self):
        ...
        self.label_input.setPlaceholderText("输入保存数据的标签值")
        self.label_input.setMaximumWidth(150)
        button_layout.addWidget(self.label_input)
        ...
        buttons = [
            ...
            ("记录手势数据", self.button4_callback)  # 合并后的按钮
        ]
    ...
    def button4_callback(self):
        """按钮4回调函数：连续记录25个电阻值和拉伸率"""
        label_value = self.label_input.text().strip()
        if not label_value:
            print("请输入标签值")
            return
            
        print(f"点击按钮：开始连续记录手势数据，标签值: {label_value}")
        self.update_status()
        
        # 开始连续记录数据
        self.start_continuous_recording(label_value)

    def start_continuous_recording(self, label_value):
        """开始连续记录数据"""
        # 初始化数据存储列表
        ...
        # 记录25个数据点
        for i in range(25):
            # 等待约100ms以达到10Hz的记录频率
            time.sleep(0.1)
            
            # 获取当前数据
            if hasattr(self, 'resistances_list') and hasattr(self, 'my_stretch_ratios'):
                resistance_data_list.append(self.resistances_list.copy())
                stretch_data_list.append(self.my_stretch_ratios.copy())
                print(f"记录数据点 {i+1}/25")
        
        # 保存电阻值数据
        resistance_filename = self.save_continuous_data_with_label(
            resistance_data_list, "resistances_list", label_value)
            
        # 保存拉伸率数据
        stretch_filename = self.save_continuous_data_with_label(
            stretch_data_list, "my_stretch_ratios", label_value)
            
        if resistance_filename and stretch_filename:
            print(f"连续数据已保存，电阻值文件: {resistance_filename}，拉伸率文件: {stretch_filename}")

    def save_continuous_data_with_label(self, data_list, file_prefix, label_value):
        """保存连续数据到CSV文件"""
        try:
            # 使用"label/label_value"作为文件夹路径
            base_folder = "label"
            ...
            # 生成文件名（带时间戳）
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            filename = f"{file_prefix}_{timestamp}_continuous.csv"
            ...
            with open(full_path, 'w', newline='') as csvfile:
                ...
                # 写入数据 - 每一行包含label值和传感器数据
                for i, data_point in enumerate(data_list):
                    formatted_data = [str(label_value)] + [f"{val}" for val in data_point]
                    writer.writerow(formatted_data)

            print(f"连续{file_prefix}已保存到文件: {full_path}")
            return full_path

        except Exception as e:
            print(f"保存连续{file_prefix}时出错: {e}")
            return None
        ...
        try:
            ...
            if frame is not None:  # 如果接收到有效数据
                ...
                if self.exoskeleton.is_calibrated():
                    ...
                    # 调用预测接口显示预测结果
                    if len(stretch_ratios) == 24:
                        if self.predict_window is None:
                            self.app_instance, self.predict_window = predict_and_show(stretch_ratios)
                        else:
                            # 更新预测窗口中的数据
                            self.predict_window.updatePrediction(stretch_ratios)
                        # 处理Qt事件循环
