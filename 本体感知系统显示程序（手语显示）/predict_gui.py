...
class ResistancePredictorUI(QWidget):
    ...
    def updatePrediction(self, sensor_data=None):
        ...
        try:
            ...
            if self.last_prediction != label:
                ...
                self.result_label.setText(f'<span style="font-weight:bold;font-family:SimHei;">当前预测结果: {label} (不稳定)</span>')
                self.result_label.setTextFormat(Qt.RichText)
                # 显示绿幕
                self.showGreenScreen()
                # 重新启动稳定计时器
                ...
                self.stable_timer.timeout.connect(lambda: self.checkStablePrediction(label))
                self.stable_timer.setSingleShot(True)
                self.stable_timer.start(1000)  # 1秒后检查是否稳定
            else:
                # 预测结果未变化，检查是否已稳定
                if self.stable_prediction == label:
                    # 结果已稳定，显示图片
                    self.result_label.setText(f'<span style="font-weight:bold;font-family:SimHei;">当前预测结果: {label} (已稳定)</span>')
                    self.result_label.setTextFormat(Qt.RichText)
    ...
    def checkStablePrediction(self, prediction):
        """检查预测结果是否稳定"""
        if self.last_prediction == prediction:
            # 预测结果在1秒内未变化，认为已稳定
            self.stable_prediction = prediction
            # 显示稳定的结果
            self.result_label.setText(f'<span style="font-weight:bold;font-family:SimHei;">当前预测结果: {prediction} (已稳定)</span>')
            self.result_label.setTextFormat(Qt.RichText)
            self.updateImage(prediction)
            self.updateLetter(prediction)
